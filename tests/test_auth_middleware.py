"""Optional bearer-token auth: off by default, enforced when MEMENTO_API_TOKEN is set."""
...
def test_no_token_env_means_no_auth(monkeypatch):
    monkeypatch.delenv("MEMENTO_API_TOKEN", raising=False)
...
def test_token_set_rejects_missing_header(monkeypatch):
    monkeypatch.setenv("MEMENTO_API_TOKEN", "s3cret")
    client = TestClient(_app())
