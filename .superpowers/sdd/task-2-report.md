...
Resolved the duplicate-query asymmetry noted in review: `MemoryManager.save()` now builds the deterministic embedding representation before duplicate detection and passes that same text into `_find_duplicate_memory_id()`.
...
```bash
uv run --extra dev pytest tests/test_manager_observe_dedupe.py tests/test_memory_representation.py -q
```
