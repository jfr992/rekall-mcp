...
tar czf "$BACKUP_DIR/auto-memory.tar.gz" \
    -C ~/.claude/projects/-Users-juanreyes-clawd-memento-mcp memory 2>/dev/null || \
...
1. Stop services:
   cd /Users/juanreyes/clawd/memento-mcp && docker-compose stop && pkill -f "uv run python -m server"
...
**Files:**
- Modify: `/Users/juanreyes/clawd/memento-mcp/.gitignore`
- Delete: `/Users/juanreyes/clawd/memento-mcp/CLAUDE_CONTINUE_PROMPT.md`

- [ ] **Step 2.1: Inspect current .gitignore**

```bash
cat /Users/juanreyes/clawd/memento-mcp/.gitignore | grep -E "^\.next|node_modules|playwright|brain.*png|tsbuildinfo" || echo "(no matches — entries needed)"
```
...
```bash
cat >> /Users/juanreyes/clawd/memento-mcp/.gitignore <<'EOF'
...
cat /Users/juanreyes/clawd/memento-mcp/.gitignore | tail -10
```
...
```bash
rm /Users/juanreyes/clawd/memento-mcp/CLAUDE_CONTINUE_PROMPT.md
...
"PreToolUse": [
  {
    "matcher": "Bash",
    "hooks": [
      { "type": "command", "command": "/Users/juanreyes/.claude/hooks/rtk-rewrite.sh" },
      { "type": "command", "command": "/Users/juanreyes/.claude/hooks/git-safety.sh" }
    ]
  }
]
```
...
"PreToolUse": [
  {
    ...
    "hooks": [
      { "type": "command", "command": "/Users/juanreyes/.claude/hooks/rtk-rewrite.sh" },
      { "type": "command", "command": "/Users/juanreyes/.claude/hooks/git-safety.sh" },
      { "type": "command", "command": "/Users/juanreyes/.claude/hooks/iac-safety.sh" }
    ]
  }
]
```
...
```bash
CLAUDE_PROJECT_DIR=/Users/juanreyes/clawd/memento-mcp \
...
"SessionStart": [
  {
    "hooks": [
      { "type": "command", "command": "/Users/juanreyes/.claude/hooks/session-context.sh" }
    ]
  }
]
```
...
echo "{\"transcript_path\":\"$TMP\",\"stop_hook_active\":false,\"cwd\":\"/Users/juanreyes/clawd/memento-mcp\"}" \
...
curl -s -X POST http://localhost:8000/api/memory/observe \
    ...
    -d "$(jq -cn \
        ...
        --arg t "decision" \
        --arg cwd "/Users/juanreyes/clawd/memento-mcp" \
...
- Hook file paths consistently absolute (`/Users/juanreyes/.claude/hooks/...`)
