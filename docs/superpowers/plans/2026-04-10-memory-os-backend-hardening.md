...
| `src/memory/scope.py` | Delete hardcoded `["acm", "acmeco"]` in `_detect_trust_boundary`. Use `trust.resolve_boundary(...)`. Add `@lru_cache` to `_git`. Add `_strip_creds()` helper; apply in `detect()` before returning `MemoryScope`. |
...
def test_no_hardcoded_client_names_in_scope_source():
    """Regression: 'acm' and 'acmeco' must not appear in scope.py."""
    src = pathlib.Path("src/memory/scope.py").read_text().lower()
    assert "acm" not in src
    assert "acmeco" not in src
...
Expected: import errors on `memory.trust`, `_strip_creds`, and the hardcoded-name regression test will fail because `scope.py` still contains `"acm"` / `"acmeco"`.
...
Expected: all pass. `test_scope.py` may need an update if it asserted the old `"acm" / "acmeco"` trust boundary logic; replace those assertions with equivalent trust.yaml-driven expectations.
