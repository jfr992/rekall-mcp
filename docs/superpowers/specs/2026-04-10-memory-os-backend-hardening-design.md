...
5. **`scope.py` hardcodes personal client names** (`["acm", "acmeco"]`) inside a module that is supposed to be a generic agent-memory fabric shared by Claude and Codex. It also leaks `repo_remote` URLs (possibly with credentials) into every memory payload.
...
def test_no_hardcoded_client_names_in_source():
    ...
    assert "acm" not in src.lower()
    assert "acmeco" not in src.lower()
