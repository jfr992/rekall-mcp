...
3. Tell the user how to get the files:
   ...
   - Write to disk: `GET /api/memory/publish?mode=dir&dest=<name>` — writes under `REKALL_PUBLISH_DIR` (default `~/.claude/publish`).

Namespace note: the tool is `mcp__<server>__publish_memory` where `<server>` is the
MCP server name in the user's Claude Code config (`rekall` if installed per the repo
