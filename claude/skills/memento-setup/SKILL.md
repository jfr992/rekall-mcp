---
name: memento-setup
description: Install or re-verify Memento's Claude Code wiring — hooks, slash commands, and settings.json entries. Idempotent. Run from inside a memento-mcp checkout. User-invocable only.
user-invocable: true
allowed-tools: Bash(*)
---
...
3. Copies `memento-restore.sh` + `memento-observe.sh` to `~/.claude/hooks/`
...
Slash commands and hooks load at session start. After install, exit Claude Code and reopen for `/memento-stats`, `/memory-recall`, etc. to become available.
