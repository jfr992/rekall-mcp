...
Codex/                      Shippable Codex bundle (NOT auto-loaded)
...
Three hooks ship in `Codex/hooks/`. They're inert until installed at `~/.Codex/hooks/` with matching `~/.Codex/settings.json` entries.
...
**Re-entrancy:** if a hook invokes `Codex -p`, that inner Codex instance fires its own hooks. Guard with `REKALL_JUDGE_INFLIGHT=1` set before exec, checked at the top of the hook. Without this, infinite hook recursion.

**Tool namespace:** the running server registers as `rekall` (in the Codex MCP config), so tools are `mcp__rekall__*` — not `mcp__memory__*`. If you copy a hook from another project, namespace-patch first.
...
- **Session-end summary** — Codex has no native end-of-session hook. Workaround would be a slash command the user types at session end. Not worth building until per-turn judge proves insufficient.
